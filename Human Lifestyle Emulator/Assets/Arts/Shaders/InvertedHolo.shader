Shader "Custom/InvertedHolo" 
{
	...
	{		
		Pass
		{
			...
			uniform float _DiffMult;
			uniform float _SpecMult;
			uniform float _RimMult;
			...
			float4 frag(vertexOutput i) : COLOR
			{
				...
				//float3 rimLighting = atten * _LightColor0.rgb * _RimColor * saturate(dot(normalDirection, lightDirection)) * pow(rim,_RimPow);
				float3 lightFinal = round(diffuseReflection*4f) / 4f;// X >> Y is the same as X / 2^Y  ERGO: this is the same as specRef*4)/
				...
				return float4(lightFinal,_Color.a);
